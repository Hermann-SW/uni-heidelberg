...
I asked Gemini on that multiplicative algebraic structure, and learned that its resulting directed acyclic graph (DAG) is well-studied in discrete mathematics and number theory. After few iterations [divisibility_dag_of_gaussian_integer_monoid.py](../scripts/divisibility_dag_of_gaussian_integer_monoid.py) showed below DAG. I am pretty sure that I will learn about that structure in Algebra 1 [next semester](https://stamm-wilbrandt.de/GraphvizFiddle/#_math).  
